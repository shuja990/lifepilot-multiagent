 ...
 * Builds the graph.
 *
 * Everything here used to be constructed at module scope, which meant importing
 * this file created a model client immediately — so a missing or misspelled API
 * key crashed the process during import, before the server could bind a port or
 * report anything useful. On a platform that restarts crashed containers, that
 * reads as a failed deploy with an SDK stack trace and no clue which variable is
 * wrong.
 *
 * Building inside the factory also means each call produces fresh agents, so
 * composing the graph more than once cannot trip ADK's "already has a parent"
 * check.
 */
function buildGraph(): SequentialAgent {
